...
class Compress {
     ...
     * @throws Exception
     */
    public function compress_image($file_url = null, $new_name_image = null, $quality = null, $pngQuality = null, $destination = null){
        
        //Send image array
        $array_img_types = array('image/gif', 'image/jpg', 'image/jpeg', 'image/pjpeg', 'image/png', 'image/x-png');
        $new_image = null;
        $last_char = null;
        $image_extension = null;
        $destination_extension = null;
        $png_compression = null;
        $real_path_file = $_SERVER['DOCUMENT_ROOT'].parse_url($file_url, PHP_URL_PATH);
        $real_destination = $_SERVER['DOCUMENT_ROOT'].parse_url($destination, PHP_URL_PATH);
        $result = array();
        $maxsize = 5245330;
        ...
        try{
            ...
            $image_data = getimagesize($file_url);
            ...
            $png_compression = (!empty($pngQuality)) ? $pngQuality : 9 ;
            
            $image_extension = pathinfo($file_url, PATHINFO_EXTENSION);
            //Verify if is sended a destination file name with extension
            $destination_extension = pathinfo($new_name_image, PATHINFO_EXTENSION); 
            //if empty
            if(empty($destination_extension)){
                $new_name_image = $new_name_image.'.'.$image_extension;
            }
            ...
            switch ($image_mime){
                ...
                case 'image/pjpeg':
                    ...
                    $new_image = imagecreatefromjpeg($file_url);
                    imagejpeg($new_image, $real_destination.$new_name_image, $quality);
                    break;
                //if is PNG and siblings
                case 'image/png':
                case 'image/x-png':
                    //Create a new png image
                    $new_image = imagecreatefrompng($file_url);
                    imagealphablending($new_image , false);
                    imagesavealpha($new_image , true);
                    imagepng($new_image, $real_destination.$new_name_image, $png_compression);
                    break;
                // if is GIF
                case 'image/gif':
                    //Create a new gif image
                    $new_image = imagecreatefromgif($file_url);
                    imagealphablending($new_image, false);
                    imagesavealpha($new_image, true);
                    imagegif($new_image, $real_destination.$new_name_image);
            }
        ...
        $result = array(
            ...
            'real_file_path' => $real_destination.$new_name_image,
            'url_file_path' => $destination.'/'.$new_name_image
        );
